...
export default function ServiceCard({ name, price, subtitle }: ServiceCardProps) {
  return (
      ...
      <div>
        ...
        {price && (
          <p className="text-muted-foreground mt-1">
            السعر: <span className="font-semibold text-primary">{price}</span>
          </p>
        )}
        {subtitle && (
          <p className="text-muted-foreground text-sm mt-0.5">
            <span className="font-medium text-primary/80">{subtitle}</span>
          </p>
        )}
      </div>
      <Button asChild className="rounded-full bg-primary hover:bg-primary/90 text-white px-6 w-full sm:w-auto shadow-sm shrink-0">
